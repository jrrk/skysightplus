...
GifFileType *DGifOpenFileHandle(int FileHandle)
{
   ...
   if (READ(GifFile,GifFile->Version, GIF_STAMP_LEN) != GIF_STAMP_LEN) {
    ...
    /* Validate the GIF version stamp. */
	/* Modified by Tim DeBenedictis (timmyd@southernstars.com) 14 March 2001 */
    if (memcmp(GIF_STAMP, GifFile->Version, GIF_VERSION_POS) != 0) {
...
*******************************************************************************/
int DGifGetGraphicControlExtension ( GifByteType *Extension, int *DisposalMethod,
	GifBooleanType *UserInputFlag, unsigned short *DelayTime,
	GifBooleanType *TransparencyFlag, GifByteType *TransparentIndex )
{
	*TransparencyFlag = Extension[0] & 0x01;
	*UserInputFlag = Extension[0] & 0x02;
	*DisposalMethod = ( Extension[0] >> 2 ) & 0x07;

	*DelayTime = ( Extension[2] << 8 ) | Extension[1];
	*TransparentIndex = Extension[3];
