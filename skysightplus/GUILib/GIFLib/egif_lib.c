...
GifFileType *EGifOpenFileHandle(int FileHandle)
{
	...
	EGifSetGifVersion ( GifFile, GIF87_STAMP ); /* GIF version handle (TCD) */
...
/******************************************************************************
*   Routine to set current GIF version. This function must be called before   *
*   the GIF screen descriptor is written.  Version consists of 6 characters   *
*   characters "GIF87a" or "GIF89a". No test is made to validate the version. *
*   Modified by Tim DeBenedictis (timmyd@southernstars.com) 14 March 2001.    *
*******************************************************************************/
void EGifSetGifVersion(GifFileType *GifFile, char *Version)
{
	memcpy ( GifFile->Version, Version, 6 );
}
...
/* First write the version prefix into the file. */
    ...
    if (WRITE(GifFile, GifFile->Version, 6 ) != 6 ) {
...
/******************************************************************************
* Put a GIF89a Graphic Control extension block into a GIF file.               *
* Implemented by Tim DeBenedictis (timmyd@southernstars.com) 14 March 2001    *
*******************************************************************************/
int EGifPutGraphicControlExtension ( GifFileType *GifFile, int DisposalMethod,
	GifBooleanType UserInputFlag, short DelayTime, GifBooleanType TransparencyFlag,
	GifByteType TransparentIndex )
{
	GifByteType Buf[4];

	Buf[0] = ( DisposalMethod << 2 ) | ( UserInputFlag << 1 ) | TransparencyFlag;
	Buf[1] = DelayTime & 0x00FF;
	Buf[2] = DelayTime >> 8;
	Buf[3] = TransparentIndex;

	return EGifPutExtension ( GifFile, GRAPHICS_EXT_FUNC_CODE, 4, Buf );
}
...
int EGifSpew(GifFileType *GifFileOut)
{
	...
	/* Modified by Tim DeBenedictis (timmyd@southernstars.com) 14 March 2001 */
    EGifSetGifVersion ( GifFileOut, gif89 ? GIF89_STAMP : GIF87_STAMP );
